...
class ListFactory
{
    ...
    {
        die("function ListFactory::ajax_user_can() must be over-ridden in a sub-class.");
    }

    /**
     * Prepares the list of items for displaying.
     *
     * @uses ListFactory::set_pagination_args()
     *
     * @since 3.1.0
     * @abstract
     */
    ...
    {
        die("function ListFactory::prepare_items() must be over-ridden in a sub-class.");
    }
    ...
    public function search_box($text, $input_id)
    {
        ...
        echo '<input type="search" id="' . esc_attr($input_id) . '" name="s" value="' . _admin_search_query() . '" />';
    ...
    protected function months_dropdown($post_type)
    {
        ...
        echo '<label for="filter-by-date" class="screen-reader-text">' . _e("Filter by date") . "</label>";
        echo '<select name="m" id="filter-by-date">';
        echo "<option" . selected($m, 0) . ' value="0">' . _e("All dates") . "</option>";
    ...
    {
        echo '<input type="hidden" name="mode" value="' . esc_attr($current_mode) . '" />';
    ...
    protected function pagination($which)
    {
        ...
        $page_links[] =
            $total_pages_before . sprintf(_x('%1$s of %2$s', "paging"), $html_current_page, $html_total_pages) . $total_pages_after;
    ...
    {
        die("function ListFactory::get_columns() must be over-ridden in a sub-class.");
    }
    ...
    /**
     * Public wrapper for ListFactory::get_default_primary_column_name().
     *
     * @since 4.4.0
     *
    ...
    protected function get_primary_column_name()
    {
        ...
        if (!isset($columns[$default])) {
            $default = ListFactory::get_default_primary_column_name();
        }
    ...
    public function print_column_headers($with_id = true)
    {
        ...
        foreach ($columns as $column_key => $column_display_name) {
            ...
            if (isset($sortable[$column_key])) {
                ...
                $column_display_name =
                    '<a href="' .
                    esc_url(remove_query_arg("action", add_query_arg(compact("orderby", "order"), $current_url))) .
                    '"><span>' .
                    $column_display_name .
    ...
    public function display()
    {
        ...
        echo '<table class="wp-list-table tripetto-wrap ' . implode(" ", $this->get_table_classes()) . '">';
        echo "<thead>";
        echo "<tr>" . $this->print_column_headers() . "</tr>";
        echo "</thead>";
        echo '<tbody id="the-list"' . ($singular ? ' data-wp-lists="list:' . $singular . '"' : "") . ">";
        echo $this->display_rows_or_placeholder();
        echo "</tbody>";
        echo "<tfoot>";
        echo "<tr>" . $this->print_column_headers(false) . "</tr>";
        echo "</tfoot>";
        echo "</table>";
    ...
    /**
     * Get a list of CSS classes for the ListFactory table tag.
     *
     * @since 3.1.0
     *
    ...
    protected function display_tablenav($which)
    {
        ...
        echo '<div class="tablenav ' . esc_attr($which) . '" style="margin-bottom: 11px;">';

        if ($this->has_items()) {
            echo '<div class="alignleft actions bulkactions">' . $this->bulk_actions($which) . "</div>";
        }
    ...
    protected function single_row_columns($item)
    {
        ...
        foreach ($columns as $column_name => $column_display_name) {
            ...
            if ("cb" === $column_name) {
                ...
                echo "</th>";
            } elseif ($cb && $column_name !== "index" && method_exists($this, "column_index")) {
                $value = method_exists($this, "column_" . $column_name)
                    ? call_user_func([$this, "column_" . $column_name], $item)
                    : $this->column_default($item, $column_name);

                echo "<td $attributes>";
                echo call_user_func([$this, "column_index"], $item, $value);
                echo "</td>";
